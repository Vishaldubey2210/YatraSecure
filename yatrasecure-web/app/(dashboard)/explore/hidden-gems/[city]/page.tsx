  ...
  if (loading) {
    return (
      <div style={{ minHeight: '80vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'var(--text2)' }}>
        ...
        <h2 style={{ fontSize: 24, fontWeight: 800, color: 'var(--text)' }}>Scanning {city} for secrets...</h2>
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '40px 20px' }}>
      ...
      <div style={{ marginBottom: 48, textAlign: 'center' }}>
        ...
        <h1 style={{ fontSize: 56, fontWeight: 950, color: 'var(--text)', letterSpacing: '-0.04em', marginBottom: 12 }}>{data.city} Secrets</h1>
        <p style={{ fontSize: 20, color: 'var(--text2)', fontStyle: 'italic', maxWidth: 600, margin: '0 auto' }}>"{data.tagline}"</p>
      </div>
        ...
        {data.gems.map((gem, i) => (
          <div key={i} style={{ 
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <h3 style={{ fontSize: 22, fontWeight: 800, color: 'var(--text)', margin: 0 }}>{gem.name}</h3>
              <span style={{ 
            ...
            <p style={{ fontSize: 15, color: 'var(--text2)', lineHeight: 1.6, margin: 0 }}>{gem.description}</p>
            ...
            <div style={{ padding: 16, borderRadius: 16, background: 'rgba(245,158,11,0.03)', border: '1px solid rgba(245,158,11,0.1)' }}>
              ...
              <p style={{ fontSize: 13, color: 'var(--text2)', margin: 0, fontStyle: 'italic' }}>{gem.travelTip}</p>
            </div>
      ...
      <div style={{ 
        ...
        <h2 style={{ fontSize: 32, fontWeight: 900, color: 'var(--text)', marginBottom: 16 }}>Found something intriguing?</h2>
        ...
        <div style={{ display: 'flex', gap: 16, justifyContent: 'center' }}>
          <button 
            ...
            style={{ padding: '16px 32px', borderRadius: 16, background: '#f59e0b', color: 'var(--text)', fontWeight: 800, border: 'none', cursor: 'pointer' }}
          >
            Start Exploration Trip
          </button>
          <button 
            onClick={() => router.back()}
            style={{ padding: '16px 32px', borderRadius: 16, background: 'var(--border)', color: 'var(--text)', fontWeight: 700, border: '1px solid rgba(255,255,255,0.1)', cursor: 'pointer' }}
          >
            Back to Guide
          </button>
        </div>
