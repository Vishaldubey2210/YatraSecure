...
import { User, Mail, MapPin, Phone, Camera, Save, Loader2, AlertCircle, Check } from "lucide-react";
...
export default function ProfilePage() {
  ...
  async function onSave() {
    ...
    try {
      const data = await fetchWithAuth("/users/me", { method: "PATCH", body: JSON.stringify(form) });
  ...
  return (
    <div className="anim-in" style={{ maxWidth: 600, margin: "0 auto" }}>
      ...
      <div className="card" style={{ padding: 28, marginBottom: 16, display: "flex", alignItems: "center", gap: 24, flexWrap: "wrap" }}>
        ...
        <div>
          <p style={{ fontSize: 18, fontWeight: 800, color: "white", marginBottom: 4 }}>{user?.username}</p>
          <p style={{ fontSize: 13, color: "#475569", marginBottom: 8 }}>{user?.email}</p>
