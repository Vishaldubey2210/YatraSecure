...
export default function MemoryLanePage() {
  ...
  if (loading) return <div style={{ padding: 80, textAlign: 'center', color: 'var(--text3)' }}>Reviving memories...</div>;

  return (
    <div className="anim-in" style={{ maxWidth: 1000, margin: '0 auto', paddingBottom: 100 }}>
      {/* Back Header */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 40 }}>
        <button onClick={() => router.back()} style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'none', border: 'none', color: 'var(--text2)', cursor: 'pointer', fontSize: 14 }}>
          <ArrowLeft style={{ width: 16, height: 16 }} /> Back to Trip
        </button>
        <div style={{ display: 'flex', gap: 12 }}>
           <button style={{ padding: '10px 20px', borderRadius: 12, background: 'var(--border)', border: '1px solid rgba(255,255,255,0.1)', color: 'var(--text)', display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
      ...
      <div style={{ 
        ...
        <div style={{ 
          ...
          <h1 style={{ fontSize: 56, fontWeight: 900, color: 'var(--text)', letterSpacing: '-0.04em', margin: 0 }}>Memory Lane</h1>
          <p style={{ fontSize: 18, color: 'var(--text2)', marginTop: 12 }}>Reflecting on the journey to {trip?.toCity}</p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20, marginBottom: 60 }}>
        {[
          ...
          <div key={i} className="glass-card" style={{ padding: 24, borderRadius: 24, textAlign: 'center', border: '1px solid rgba(255,255,255,0.05)' }}>
            <div style={{ width: 44, height: 44, borderRadius: 12, background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
              <item.icon style={{ width: 22, height: 22, color: item.color }} />
            </div>
            <div style={{ fontSize: 11, color: 'var(--text3)', fontWeight: 800, textTransform: 'uppercase', marginBottom: 4 }}>{item.label}</div>
      ...
      <div style={{ 
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24 }}>
          ...
          <h2 style={{ fontSize: 32, fontWeight: 800, color: 'var(--text)', margin: 0 }}>The Trip Memoir</h2>
          ...
          {story ? (
            ...
            <>
              <p style={{ color: 'var(--text2)', fontSize: 15, maxWidth: 500 }}>
              ...
              <button 
                ...
                style={{ 
                  ...
                  border: 'none', color: 'var(--text)', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 10, 
          ...
          {story && (
             <button onClick={() => setStory('')} style={{ color: 'var(--text3)', fontSize: 12, background: 'none', border: 'none', cursor: 'pointer', marginTop: 20 }}>Regenerate Story</button>
          )}
        </div>
      </div>
      ...
      <div style={{ marginBottom: 40 }}>
        <h3 style={{ fontSize: 24, fontWeight: 800, color: 'var(--text)', marginBottom: 24, textAlign: 'center' }}>Travel Analytics</h3>
      ...
      <div style={{ textAlign: 'center', marginTop: 80, padding: 40, borderTop: '1px solid rgba(255,255,255,0.05)' }}>
        ...
        <p style={{ color: 'var(--text3)', fontSize: 14 }}>YatraSecure Memory Engine v1.0 • Built with Love</p>
      </div>
    </div>
  );
}
